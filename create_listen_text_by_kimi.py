...
def generate(my_api_key, my_base_url, my_model, filepath, levelname):
    ...
    while (not correct_format_flag):
        ...
        user_prompt = """generate one similar level `TRANSCRIPTION DES DOCUMENTS AUDIO` (audio scripts) and one corresponding exercise (which should include one to three sub-exercise) and give the questions and answers in this json format 
                `[
                    {
                        "question": "...",
                        "options": [
                ...
                ]`. In the output, for the audio script part, start with overhead `<script>` and end with `</script>`, for the exercise json part, start with overhead `<exercise>` and end with `</exercise>`
